...
import { Filter, MapPin, AlertTriangle, Map } from 'lucide-react';

interface MapViewProps {
  tickets: Ticket[];
}

export const MapView: React.FC<MapViewProps> = ({ tickets }) => {
  ...
  const mapTickets = useMemo(() => {
      return tickets.filter(t => {
          if (!t.location || !t.location.includes(',')) return false;
  ...
  // Limit to 20 markers to avoid URL length limit
  const getStaticMapUrl = () => {
      if (mapTickets.length === 0) return null;

      const markers = mapTickets.slice(0, 20).map((t, idx) => {
          const [lat, lon] = t.location!.split(',').map(s => s.trim());
          // pt=lon,lat,style
          // styles: pm2 (placemark), rd (red), m (medium) -> pm2rdm
          // Let's color code: red for critical, green for resolved
          let color = 'bl'; // blue default
          if (t.priority === 'critical' || t.priority === 'high') color = 'rd'; // red
          if (t.status === 'resolved') color = 'gn'; // green
          if (t.status === 'new') color = 'yw'; // yellow (orangeish)
          
          return `${lon},${lat},pm2${color}m${idx+1}`; // numbered marker
      }).join('~');

      // Center map on the first ticket or average? Let's auto-center by Yandex (omit ll/z if pt is present, but usually better to provide ll)
      // We will let Yandex auto-scale based on pt
      return `https://static-maps.yandex.ru/1.x/?l=map&pt=${markers}&size=600,400`;
  };
    ...
    <div className="h-full p-6 bg-slate-50 flex flex-col">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div>
              ...
              <p className="text-slate-500 text-sm mt-1">География инцидентов (Топ-20 актуальных)</p>
          </div>
      ...
      <div className="flex-1 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex relative">
          {mapTickets.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
                  ...
                  <p>Нет заявок с координатами для отображения</p>
              </div>
          ) : (
              <>
                ...
                {/* Sidebar List */}
                <div className="w-80 border-l border-slate-200 bg-white overflow-y-auto p-4 hidden lg:block">
                    ...
                    <div className="space-y-3">
                        {mapTickets.slice(0, 20).map((t, idx) => (
                            <div key={t.id} className="flex gap-3 p-3 rounded-lg border border-slate-100 hover:border-purple-200 hover:bg-purple-50 transition-all cursor-pointer group">
